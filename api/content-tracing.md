...
| Version(s) | Changes |
| --- | --- |
| ```markdown None ``` | The `options` parameter now accepts `TraceConfig` in addition to `TraceCategoriesAndOptions`. |
| ```markdown None ``` | [  This function now returns a callback `Promise<void>`.  ](../breaking-changes.md#api-changed-callback-based-versions-of-promisified-apis) |
...
| --- | --- |
| ```markdown None ``` | [  This method now returns a Promise instead of using a callback function.  ](../breaking-changes.md#api-changed-callback-based-versions-of-promisified-apis) |
| ```markdown None ``` | The `resultFilePath` parameter is now optional. |
...
Enable [heap profiling](https://chromium.googlesource.com/chromium/src/+/lkgr/docs/memory-infra/heap_profiler.md) for MemoryInfra traces. Equivalent to the `--memlog` switch in Chrome.
...
1. Download the breakpad symbols for your Electron version from the Electron GitHub [releases](https://github.com/electron/electron/releases)
...
![Screenshot showing how to view a heapdump in Chromium's tracing view](https://www.electronjs.org/assets/images/viewing-heap-dumps-8621f537414e3056f836ac73cea2dbd9.png)
