...
Send a message to the child process, optionally transferring ownership of zero or more [`MessagePortMain`](message-port-main.md) objects.
...
This behaves the same as the [`login` event on `app`](app.md#event-login) but is scoped to the individual utility process instance.
