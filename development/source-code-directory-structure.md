...
You may need to become familiar with [Chromium's multi-process architecture](https://www.chromium.org/developers/design-documents/multi-process-architecture/) to understand the source code better.

## Project structure

Electron is a complex project containing multiple upstream dependencies, which are tracked in source control via the [`DEPS`](https://github.com/electron/electron/blob/v44.4.2/DEPS) file. When [initializing a local Electron checkout](build-instructions-gn.md), Electron's source code is just one of many nested folders within the project root.
...
For all code outside of `src/electron`, Electron-specific code changes are maintained via git patches. See the [Patches](patches.md) development guide for more information.
