...
In Electron, processes communicate by passing messages through developer-defined "channels" with the [`ipcMain`](../api/ipc-main.md) and [`ipcRenderer`](../api/ipc-renderer.md) modules. These channels are **arbitrary** (you can name them anything you want) and **bidirectional** (you can use the same channel name for both modules).
...
Before proceeding to implementation details, you should be familiar with the idea of using a [preload script](process-model.md#preload-scripts) to import Node.js and Electron modules in a context-isolated renderer process.

- For a full overview of Electron's process model, you can read the [process model docs](process-model.md).
- For a primer into exposing APIs from your preload script using the `contextBridge` module, check out the [context isolation tutorial](context-isolation.md).

## Pattern 1: Renderer to main (one-way)

To fire a one-way IPC message from a renderer process to the main process, you can use the [`ipcRenderer.send`](../api/ipc-renderer.md) API to send a message that is then received by the [`ipcMain.on`](../api/ipc-main.md) API.
...
A common application for two-way IPC is calling a main process module from your renderer process code and waiting for a result. This can be done by using [`ipcRenderer.invoke`](../api/ipc-renderer.md#ipcrendererinvokechannel-args) paired with [`ipcMain.handle`](../api/ipc-main.md#ipcmainhandlechannel-listener).
...
When sending a message from the main process to a renderer process, you need to specify which renderer is receiving the message. Messages need to be sent to a renderer process via its [`WebContents`](../api/web-contents.md) instance. This WebContents instance contains a [`send`](../api/web-contents.md#contentssendchannel-args) method that can be used in the same way as `ipcRenderer.send`.
...
- Pass a [MessagePort](message-ports.md) from the main process to both renderers. This will allow direct communication between renderers after the initial setup.
