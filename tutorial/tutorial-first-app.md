...
Because Electron's main process is a Node.js runtime, you can execute arbitrary Node.js code with the `electron` command (you can even use it as a [REPL](repl.md)). To execute this script, add `electron .` to the `start` command in the [`scripts`](https://docs.npmjs.com/cli/v7/using-npm/scripts) field of your package.json. This command will tell the Electron executable to look for the main script in the current directory and run it in dev mode.
...
Now that you have a web page, you can load it into an Electron [BrowserWindow](../api/browser-window.md). Replace the contents of your `main.js` file with the following code. We will explain each highlighted block separately.
...
- [app](../api/app.md), which controls your application's event lifecycle.
- [BrowserWindow](../api/browser-window.md), which creates and manages app windows.
...
On Windows and Linux, closing all windows will generally quit an application entirely. To implement this pattern in your Electron app, listen for the app module's [`window-all-closed`](../api/app.md#event-window-all-closed) event, and call [`app.quit()`](../api/app.md#appquit) to exit your app if the user is not on macOS.
...
To implement this feature, listen for the app module's [`activate`](../api/app.md#event-activate-macos) event, and call your existing `createWindow()` method if no BrowserWindows are open.
...
> - [DevTools Extensions](devtools-extension.md)
